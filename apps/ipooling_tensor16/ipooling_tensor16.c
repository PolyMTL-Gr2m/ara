...
void imax_pool_vec_1xC_2x2(int16_t *o, int16_t *i, int64_t R, int64_t C, int64_t W, int64_t F, int64_t stride) {
	...
	asm volatile("vsetvli zero, %0, e16, m4, ta, ma" ::"r"(TILE_SIZE >> 1)); // we only load one of two values 
	
	for (int c = 0 ; c < C ; c += TILE_SIZE) // IF CONVOLUTION NEED TO BE TILED (C > TILE_SIZE)
	{
		if(c > C - TILE_SIZE)
			asm volatile("vsetvli zero, %0, e16, m4, ta, ma" ::"r"((C % TILE_SIZE) >> 1)); // we only load one of two values 

		int16_t * i_ = i + c; 			// input pointer realtive to the tile (constant throughout the tile)
		int16_t * o_ = o + (c >> 1);			// output pointer relative to the tile	
		
		//channel and height loop are fused to avoid branches
		for (int r = 0 ; r < R * W ; r += block_size_2x2){
			
		  	// Load F row of size C
		  	asm volatile("vlse16.v v16, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += 1;
		  	asm volatile("vlse16.v v20, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += C - 1;
		  	// next line
		  	asm volatile("vlse16.v v24, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += 1;
		  	asm volatile("vlse16.v v28, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += C - 1;
...
void imax_pool_vec_1xC_3x3(int16_t *o, int16_t *i, int64_t R, int64_t C, int64_t W, int64_t F, int64_t stride) {
	...
	// vsetvli C (in order to load everything)
	asm volatile("vsetvli zero, %0, e16, m2, ta, ma" ::"r"(TILE_SIZE_3x3 / 3)); // we only load one of two values 
	
	for (int c = 0 ; c < C ; c += TILE_SIZE_3x3) // IF CONVOLUTION NEED TO BE TILED (C > TILE_SIZE)
	{
		if(c > C - TILE_SIZE_3x3)
			asm volatile("vsetvli zero, %0, e16, m2, ta, ma" ::"r"((C % TILE_SIZE_3x3) / 3)); // we only load one of two values 

		int16_t * i_ = i + c; 			// input pointer realtive to the tile (constant throughout the tile)
		int16_t * o_ = o + c / 3;			// output pointer relative to the tile	
		
		//channel and height loop are fused to avoid branches
		for (int r = 0 ; r < R * W ; r += block_size_3x3){
			
		  	// Load F row of size C
		  	asm volatile("vlse16.v v10, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += 1;
		  	asm volatile("vlse16.v v12, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += 1;
		  	asm volatile("vlse16.v v14, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += C - 2;
		  	
		  	// next line
		  	asm volatile("vlse16.v v16, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += 1;
		  	asm volatile("vlse16.v v18, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += 1;
		  	asm volatile("vlse16.v v20, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += C - 2;
		  	
		  	// next line
		  	asm volatile("vlse16.v v22, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += 1;
		  	asm volatile("vlse16.v v24, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += 1;
		  	asm volatile("vlse16.v v26, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += C - 2;
...
void iavg_pool_vec_1xC_2x2(int16_t *o, int16_t *i, int64_t R, int64_t C, int64_t W, int64_t F, int64_t stride) {
	...
	for (int c = 0 ; c < C ; c += TILE_SIZE) // IF CONVOLUTION NEED TO BE TILED (C > TILE_SIZE)
	{
		...
		// vsetvli C (in order to load everything)
		asm volatile("vsetvli zero, %0, e16, m2, ta, ma" ::"r"(vlen)); // we only load one of two values 
		...
		for (int r = 0 ; r < R * W ; r += block_size_2x2){
		 	...
		 	asm volatile("vsetvli zero, %0, e32, m4, ta, ma" ::"r"(vlen)); // we only load one of two values 
...
void iavg_pool_vec_1xC_3x3(int16_t *o, int16_t *i, int64_t R, int64_t C, int64_t W, int64_t F, int64_t stride) {
	...
	for (int c = 0 ; c < C ; c += TILE_SIZE_3x3) // IF CONVOLUTION NEED TO BE TILED (C > TILE_SIZE)
	{
		...
		for (int r = 0 ; r < R * W ; r += block_size_3x3){
 			...
 			asm volatile("vsetvli zero, %0, e32, m4, ta, ma" ::"r"(vlen)); // all our results are on 16b

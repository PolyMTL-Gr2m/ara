...
int main() {
  ...
  for(int size = 8 ; size <= I_MAX ; size = size * 2){
	  ...
	  printf("----------------------------------------------------------------\n");
		...
		int16_t output[width * height * channels]; // keep empty space at max size (just for the test)
		int16_t golden_output[width * height  * channels];
	  ...
	  printf("Computing result...\n");
	  	start_timer();
		iReLu_tensor16(output, input, height, width, channels);
		stop_timer();

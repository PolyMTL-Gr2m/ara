...
/*void iconv2d_tensor32_vec_4xC_slice_init_3x3(int64_t *o, int64_t C) {
  // Helper variables
  int64_t ldo = C << 3;
...
void iconv2d_tensor32_vec_4xC_3x3(int32_t *o, int32_t *i, int32_t *f, int64_t C, int64_t W,
  ...
  asm volatile("lw %1, (%0); sub %0, %0, %2" : "+&r"(f_), "=&r"(t2) : "r"(next_column)); 
  ...
  asm volatile("lw %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t0) : "r"(ldf));
  asm volatile("lw %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t1) : "r"(ldf));
  asm volatile("lw %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t2) : "r"(next_kernel));
...
/*void iconv2d_tensor32_vec_4xC_slice_move_3x3(int64_t C, int64_t F) { //could be implemented direclty in main convolution function conv2d_vec
  // Move C+F-1 elements
  asm volatile("vsetvli zero, %0, e32, m2, ta, ma" ::"r"(C + F - 1));

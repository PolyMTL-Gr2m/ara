...
N_pad = N + 2*padding
L_pad = L + 0*padding
#assert(M % 1 == 0), "Output image dimension must be divisible by 4, pad the input image accordingly"
#assert(N % 1 == 0), "Output image dimension must be divisible by 4, pad the input image accordingly"
#assert(L / filter_size != 1), "Depth of tensor must be same depth as the filter"

# Generate a random int64 input padded image
tensor = np.around(rand_tensor(1, L_pad, M_pad, N_pad,1)).astype(np.uint32)
np.random.shuffle(tensor.flat)

# Generate a random int64 filter
gen_filter = np.around(rand_tensor(num_filter, L, filter_size, filter_size, 0)).astype(np.uint32)
np.random.shuffle(gen_filter.flat)

# Create the empty o matrix
empty_o = np.zeros((1, M, N)).astype(np.uint32)
...
result = np.zeros((num_filter, M, N)).astype(np.uint32) #Num_filter x M x N
for num in range(num_filter):
	for plane in range(L):
		result[num,:,:] += np.around(scipy.signal.convolve2d(np.flip(gen_filter[num,plane,:,:]), tensor[0,plane,:,:], 'valid')).astype(np.uint32) # https://stackoverflow.com/questions/41613155/what-does-scipy-signal-convolve2d-calculate

# Calculate a checksum
checksum = np.sum(result, dtype=np.uint32)

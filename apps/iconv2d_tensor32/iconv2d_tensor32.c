...
void iconv2d_tensor32(int32_t *o, int32_t *i, int32_t *f, int64_t H_in, int64_t W_in, int64_t C_in, int64_t F, int64_t C_out) {
  ...
  for(int64_t c = 0; c < C_out; c++) {
	  ...
	  // Iterate over the output rows
		...
		switch (F){
			 case 1:
					iconv2d_tensor32_vec_4xC_1x1(o_, i_, f_, H_in, W_in, C_in, F);
				break;

			 case 3:
					iconv2d_tensor32_vec_6xC_3x3(o_, i_, f_, H_in, W_in, C_in, F);
				break;
				
			 case 5:
					iconv2d_tensor32_vec_6xC_5x5(o_, i_, f_, H_in, W_in, C_in, F);
				break;
				
			 case 7:
					iconv2d_tensor32_vec_4xC_7x7(o_, i_, f_, H_in, W_in, C_in, F);
				break;
		}
...
for (int c = 0 ; c < (C - F + 1) ; c += TILE_SIZE_OUT) // IF CONVOLUTION NEED TO BE TILED (C > TILE_SIZE)
{
	...
	for (int r = 0; r < R ; r += block_size_1x1)
	{
		...
		asm volatile("lw %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t0) : "r"(next_kernel));

...
// ulppack_conv2d_vec16_7x7 with 2.1 loop unrolled on 4 cores (Cin == 32)
...
uint16_t *f_loop = f_packed;
	...
	for (int width = 0 ; width < (W_in - 6) ; width += VLEN_MF2_OUT) // IF CONVOLUTION NEED TO BE TILED (C > VLEN_WIDE)
	{
		...
		} else if (get_hartid()==1){
		for(int channels = 4 ; channels < 8 ; channels ++){
		...
		} else if (get_hartid()==2){
		for(int channels = 8 ; channels < 12 ; channels ++){
		...
		for(int channels = 12 ; channels < 16 ; channels ++){
...
// ulppack_conv2d_vec8_7x7 with 2.1 loop unrolled on 4 cores (Cin == 32)

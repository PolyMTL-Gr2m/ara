...
extern int16_t i[] __attribute__((aligned(4 * NR_LANES))); // [M x N x L]
extern int16_t f[] __attribute__((aligned(4 * NR_LANES))); // [K x F x F x L]

// o points to a [(M + F - 1) x (N - F + 1) x K] memory vector which is set to 0 at the start
		
extern int16_t o[] __attribute__((aligned(4 * NR_LANES)));

// expected output (golden_o) are size of [(M + F - 1) x (N - F + 1) x K]

extern int16_t golden_o_1[] __attribute__((aligned(4 * NR_LANES)));
extern int16_t golden_o_3[] __attribute__((aligned(4 * NR_LANES)));
extern int16_t golden_o_5[] __attribute__((aligned(4 * NR_LANES)));
extern int16_t golden_o_7[] __attribute__((aligned(4 * NR_LANES)));
...
void print_16btensor(int16_t *tensor, uint64_t num_rows,
...
int16_t * golden_o;
	...
	printf("Result (16b) is an output of [1 x %i x %i x %i] \n", K, size - F + 1, size - F + 1);
	printf("----------------------------------------------------------------\n");
	...
	int16_t input[width * height * channels];
	int16_t filter[filters * F * F * channels];
	int16_t output[width * height * filters];
	int16_t golden_output[(width - F + 1) * (height- F + 1) * filters];
	...
	start_timer();
	#endif			
	iconv2d_tensor16(output, input, filter, height, width, channels, F, filters);
	#ifndef SPIKE
	stop_timer();
	#endif
	...
	int error = verify_16btensor(output, golden_output, (height - F + 1), (width - F + 1), filters);
	
	//print_16btensor(input, height, width, channels);
	//print_16btensor(output, (height - F + 1), (width - F + 1), filters);
...
for(int64_t F = 7 ; F <= F_MAX ; F += 2){
	...
	start_timer();			
	#endif
	iconv2d_tensor16_wide(output, input, filter, height, width, channels, F, filters);
	#ifndef SPIKE
	stop_timer();
	#endif

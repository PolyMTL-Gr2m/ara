...
extern float f[] __attribute__((aligned(4 * NR_LANES)));        // [ F*F ]
...
int verify_64btensor(float *tensor1, float *tensor2, int64_t R,
                  int64_t C, int64_t D) {
...
void print_64btensor(float const *tensor, uint64_t num_rows,
                  uint64_t num_columns, uint64_t num_depth) {
  printf("0x%8X\n", (uint64_t)tensor);
  for (uint64_t k = 0; k < num_depth; ++k) {
  	for (uint64_t i = 0; i < num_rows; ++i) {
    	for (uint64_t j = 0; j < num_columns; ++j) {
      printf("%10f ", tensor[(i+k*num_rows) * num_columns  + j ]);
    	}
    	printf("\n");
  	 }
  	 printf("\n");
...
void print_f32tensor(float const *tensor, uint64_t num_rows,
                  uint64_t num_columns, uint64_t num_depth) {
  printf("0x%8X\n", (uint64_t)tensor);
  for (uint64_t k = 0; k < num_depth; ++k) {
  	for (uint64_t i = 0; i < num_rows; ++i) {
...
int main() {
  ...
  */stop_timer();
  ...
  int error = verify_f32tensor(o, golden_o, M, N, K);
  if (error != 0) {
    printf("Fail.\n");
  } else {
    printf("Passed.\n");

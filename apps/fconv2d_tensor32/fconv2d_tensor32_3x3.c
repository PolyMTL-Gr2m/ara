...
void fconv2d_tensor32_3x3(float *o, float *i, float *f, int64_t R, int64_t C, int64_t W,
                int64_t F, int64_t K) {
  ...
  for(int64_t k = 0; k < K; k++) {
	  ...
	  fconv2d_tensor32_vec_4xC_slice_preload_3x3(i_, C, F);
	  // The first F-1 rows have already been loaded by
	  // conv2d_vec_4xC_slice_preload_3x3()
	  float *i__ = i_ + (F - 1) * (C + F - 1);
	  fconv2d_tensor32_vec_4xC_3x3(o_, i__, f_, C, W, F);
...
/*void fconv2d_tensor32_vec_4xC_slice_init_3x3(int64_t *o, int64_t C) {
  // Helper variables
  int64_t ldo = C << 3;
...
// Load 4 rows of the output matrix
void fconv2d_tensor32_vec_4xC_slice_preload_3x3(float *i, int64_t C, int64_t F) {
  // Helper variables
  int64_t ldi = (C + F - 1) << 2;
  ...
  asm volatile("vle32.v v10, (%0); add %0, %0, %1" : "+&r"(i) : "r"(next_plane)); 
  
  asm volatile("vle32.v v24, (%0); add %0, %0, %1" : "+&r"(i) : "r"(ldi)); //second plabe
  asm volatile("vle32.v v26, (%0); add %0, %0, %1" : "+&r"(i) : "r"(next_plane));
	  
  asm volatile("vle32.v v28, (%0); add %0, %0, %1" : "+&r"(i) : "r"(ldi)); //third plane
  asm volatile("vle32.v v30, (%0); add %0, %0, %1" : "+r"(i)); 
}

// Calculate 4 output matrix rows
void fconv2d_tensor32_vec_4xC_3x3(float *o, float *i, float *f, int64_t C, int64_t W,
                        int64_t F) {
  ...
  int64_t ldo = C << 2;
  int64_t ldi = (C + F - 1) << 2;
  int64_t next_plane = (C + F - 4)*(C + F - 1) << 2; 
  int64_t ldf = F << 2;
  int64_t next_column = (F * (F - 1) - 1) << 2;
  int64_t next_kernel = 1 << 2;
  float *f_;
  ...
  asm volatile("flw %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&f"(t0) : "r"(ldf));
  asm volatile("flw %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&f"(t1) : "r"(ldf));
  asm volatile("flw %1, (%0); sub %0, %0, %2" : "+&r"(f_), "=&f"(t2) : "r"(next_column)); 
  ...
  asm volatile("vle32.v v18, (%0); add %0, %0, %1" : "+&r"(i) : "r"(next_plane)); 
  ...
  asm volatile("vfmacc.vf v6, %0, v20" ::"f"(t1));
  
  asm volatile("vslidedown.vi v22, v18, 1");
  asm volatile("vfmacc.vf v6, %0, v22" ::"f"(t2));
  
  // Fetch the third column of the filter, and start calculating its
  // contributions on the output rows To do so, slide down the input rows by one
  asm volatile("flw %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&f"(t0) : "r"(ldf));
  asm volatile("flw %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&f"(t1) : "r"(ldf));
  asm volatile("flw %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&f"(t2) : "r"(next_kernel));
 ...
 if (W >= 2){
	  ...
	  asm volatile("vfmacc.vf v2, %0, v26" ::"f"(t0));
	  ...
	  asm volatile("vle32.v v8,  (%0); add %0, %0, %1" : "+&r"(i) : "r"(ldi));
	  asm volatile("vfmacc.vf v0, %0, v26" ::"f"(t1));  
	  asm volatile("vfmacc.vf v2, %0, v12" ::"f"(t1));
	  asm volatile("vfmacc.vf v4, %0, v14" ::"f"(t1));
	  asm volatile("vfmacc.vf v6, %0, v8"  ::"f"(t1));

	  asm volatile("vle32.v v10, (%0); add %0, %0, %1" : "+&r"(i) : "r"(next_plane)); 
	  asm volatile("vfmacc.vf v0, %0, v12" ::"f"(t2));  
	  asm volatile("vfmacc.vf v2, %0, v14" ::"f"(t2));
	  asm volatile("vfmacc.vf v4, %0, v8"  ::"f"(t2));
	  asm volatile("vfmacc.vf v6, %0, v10" ::"f"(t2));
 	...
 	if (W == 3){ 
		  ...
		  asm volatile("vle32.v v12, (%0); add %0, %0, %1" : "+&r"(i) : "r"(ldi));
		  asm volatile("vfmacc.vf v0, %0, v28" ::"f"(t0));  
		  asm volatile("vfmacc.vf v2, %0, v30" ::"f"(t0));
		  ...
		  asm volatile("vfmacc.vf v0, %0, v30" ::"f"(t1));  
		  asm volatile("vfmacc.vf v2, %0, v12" ::"f"(t1));
		  asm volatile("vfmacc.vf v4, %0, v14" ::"f"(t1));
		  asm volatile("vfmacc.vf v6, %0, v20" ::"f"(t1));
		  ...
		  asm volatile("vfmacc.vf v6, %0, v24" ::"f"(t1));
		  
		  asm volatile("vslidedown.vi v26, v22, 1");
		  asm volatile("vfmacc.vf v6, %0, v26" ::"f"(t2));

		  
		  asm volatile("flw %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&f"(t0) : "r"(ldf)); 
		  asm volatile("flw %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&f"(t1) : "r"(ldf));
		  asm volatile("flw %1, (%0)" : "+&r"(f_), "=&f"(t2));
  ...
  asm volatile("vse32.v  v2, (%0); add %0, %0, %1" : "+&r"(o) : "r"(ldo));
...
/*void fconv2d_tensor32_vec_4xC_slice_move_3x3(int64_t C, int64_t F) { //could be implemented direclty in main convolution function conv2d_vec
  // Move C+F-1 elements
  asm volatile("vsetvli zero, %0, e32, m2, ta, ma" ::"r"(C + F - 1));

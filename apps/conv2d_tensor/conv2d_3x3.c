...
void conv2d_3x3(int64_t *o, int64_t *i, int64_t *f, int64_t R, int64_t C, int64_t W,
  ...
  //helper variable
  int64_t f_volume = F*F*W; //number of elements in one kernel
  int64_t i_size = R*C;
...
void conv2d_vec_4xC_3x3(int64_t *o, int64_t *i, int64_t *f, int64_t C, int64_t W,
  ...
  asm volatile("vsetvli zero, %0, e64, m2, ta, ma" ::"r"(C + F - 1)); // A VOOOOOOIR
  ...
  asm volatile("vle64.v v16, (%0); add %0, %0, %1" : "+&r"(i) : "r"(ldi));         // v20
  asm volatile("vle64.v v18, (%0); add %0, %0, %1" : "+&r"(i));  // v22
  ...
  asm volatile("ld %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t0) : "r"(ldf)); //load t0 from middle perpendicular plane of the kernel 
  ...
  asm volatile("vmacc.vx v0, %0, v12" ::"r"(t2)); //third index of input matrix add up 
  asm volatile("vmacc.vx v2, %0, v12" ::"r"(t1)); //
  asm volatile("vmacc.vx v4, %0, v12" ::"r"(t0));
 ...
 if (W >= 2){
	  ...
	  asm volatile("vle64.v v14, (%0); add %0, %0, %1" : "+&r"(i) : "r"(ldi));//second plabe
	  asm volatile("vle64.v v8, (%0); add %0, %0, %1" : "+&r"(i) : "r"(ldi));//second plabe   /
	  asm volatile("vle64.v v10, (%0); add %0, %0, %1" : "+&r"(i) : "r"(next_plane));//second plabe 
	  
	  //first matrix 2nd plane
	  asm volatile("ld %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t0) : "r"(ldf)); //voir adresse f
	  asm volatile("vmacc.vx v0, %0, v24"  ::"r"(t0));  
	  asm volatile("vmacc.vx v2, %0, v26"  ::"r"(t0));
	  asm volatile("vmacc.vx v4, %0, v12" ::"r"(t0));
	  asm volatile("vmacc.vx v6, %0, v14" ::"r"(t0));
	  
	  //fetch F0 and compute on seconde plane
	  asm volatile("ld %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t1) : "r"(ldf)); //voir adresse f
	  asm volatile("vmacc.vx v0, %0, v26"  ::"r"(t1));  
	  asm volatile("vmacc.vx v2, %0, v12" ::"r"(t1));
	  asm volatile("vmacc.vx v4, %0, v14" ::"r"(t1));
	  asm volatile("vmacc.vx v6, %0, v8" ::"r"(t1));
	  
	  //fetch F0 on seconde plane
	  asm volatile("ld %1, (%0); sub %0, %0, %2" : "+&r"(f_), "=&r"(t2) : "r"(next_column)); //voir adresse f
	  asm volatile("vmacc.vx v0, %0, v12" ::"r"(t2));  
	  asm volatile("vmacc.vx v2, %0, v14" ::"r"(t2));
	  ...
	  asm volatile("ld %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t0) : "r"(ldf)); //VOIR ADRESSE T10
	  ...
	  //2nd plane
	  asm volatile("vslidedown.vi v14, v14, 1");
	  asm volatile("vmacc.vx v2, %0, v14" ::"r"(t2)); //third index of input matrix add up 
	  asm volatile("vmacc.vx v4, %0, v14" ::"r"(t1)); //
	  asm volatile("vmacc.vx v6, %0, v14" ::"r"(t0));
 	...
 	if (W == 3){ 
		  ...
		  asm volatile("vle64.v v12, (%0); add %0, %0, %1" : "+&r"(i) : "r"(ldi));//third plane 
		  asm volatile("vle64.v v14, (%0); add %0, %0, %1" : "+&r"(i) : "r"(ldi));//third plane  
		  asm volatile("vle64.v v20, (%0); add %0, %0, %1" : "+&r"(i) : "r"(ldi));//third plane  
		  asm volatile("vle64.v v22, (%0); add %0, %0, %1" : "+&r"(i) : "r"(ldi));//third plane
		  ...
		  // KEEP v16-17-18 untouched to write them back after the block is computed
		  asm volatile("vslidedown.vi v24, v20, 1");
		  asm volatile("vmacc.vx v4, %0, v24" ::"r"(t2));
		  asm volatile("vmacc.vx v6, %0, v24" ::"r"(t1));
		  
		  // KEEP v19-20-18 untouched to write them back after the block is computed
...
void conv2d_vec_4xC_slice_move_3x3(int64_t C, int64_t F) { //not used implemented direclty in conv2d_vec_4
  // Move C+F-1 elements
  asm volatile("vsetvli zero, %0, e64, m2, ta, ma" ::"r"(C + F - 1));
  // Move the last floor(F/2) + 1 input rows
  
  //order of instruction does matter (will change to be more coherent with the register)

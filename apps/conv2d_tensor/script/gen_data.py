...
tensor = np.around(rand_tensor(1, L_pad, M_pad, N_pad,1)).astype(np.int64)
np.random.shuffle(tensor.flat)

# Generate a random int64 filter
gen_filter = np.around(rand_tensor(num_filter, L, filter_size, filter_size, 0)).astype(np.int64)
np.random.shuffle(gen_filter.flat)

# Create the empty o matrix
empty_o = np.zeros((L, M, N)).astype(np.int64)
...
#gen_filter[1,0,:,:] = np.zeros((1, filter_size, filter_size)).astype(np.int64)
#gen_filter[1,1,:,:] = np.zeros((1, filter_size, filter_size)).astype(np.int64)
#gen_filter[1,2,:,:] = np.zeros((1, filter_size, filter_size)).astype(np.int64)
...
#tensor[0,0,:,:] = np.zeros((1, M_pad, N_pad)).astype(np.int64)
...
result = np.zeros((num_filter, M, N)).astype(np.int64) #Num_filter x M x N
for num in range(num_filter):
	for plane in range(L):
		result[num,:,:] += np.around(scipy.signal.convolve2d(np.flip(gen_filter[num,plane,:,:]), tensor[0,plane,:,:], 'valid')).astype(np.int64) # https://stackoverflow.com/questions/41613155/what-does-scipy-signal-convolve2d-calculate

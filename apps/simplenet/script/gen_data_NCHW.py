...
tensor = np.around(rand_tensor(1, L_pad, M_pad, N_pad, -128)).astype(np.int8) << 7
np.random.shuffle(tensor.flat)

# Generate a random int64 filter
gen_filter = np.around(rand_tensor(num_filter, L, filter_size, filter_size, -128)).astype(np.int8) 
...
# if wide input (8b) -> output (8b)
	...
	gen_filter_conv = gen_filter.astype(np.int8)
	
	# Create the empty o matrix
	empty_o = np.zeros((num_filter, M, N)).astype(np.int8)
	
	result = np.zeros((num_filter, M, N)).astype(np.int8) #Num_filter x M x N
	for num in range(num_filter):
		for plane in range(L):
			result[num,:,:] += np.around(scipy.signal.convolve2d(np.flip(gen_filter_conv[num,plane,:,:]), tensor_conv[0,plane,:,:], 'valid')).astype(np.int8) # https://stackoverflow.com/questions/41613155/what-does-scipy-signal-convolve2d-calculate
	
	# reshape into NWHC format
if memlayout == "NHWC":
	result_NWHC = np.zeros((M, N, num_filter)).astype(np.int8) 
	tensor_NWHC = np.zeros((1, M_pad, N_pad, L)).astype(np.int8) 
	gen_filter_NWHC = np.zeros((num_filter,filter_size, filter_size, L_pad)).astype(np.int8) 
...
emit_8b("i", tensor, 'NR_LANES*4')
emit_8b("f", gen_filter, 'NR_LANES*4')
emit_8b("o", empty_o, 'NR_LANES*4')
emit_8b("golden_o", result, 'NR_LANES*4')

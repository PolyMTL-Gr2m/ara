...
extern float i[] __attribute__((aligned(4 * NR_LANES))); // [ (M+floor(F/2)) * (N+floor(F/2)) ]
extern float o[] __attribute__((aligned(4 * NR_LANES)));        // [ M*N ]

extern float golden_o_max_2 [] __attribute__((aligned(4 * NR_LANES))); // [ M*N ]
extern float golden_o_avg_2 [] __attribute__((aligned(4 * NR_LANES))); // [ M*N ]

extern float golden_o_max_3 [] __attribute__((aligned(4 * NR_LANES))); // [ M*N ]
extern float golden_o_avg_3 [] __attribute__((aligned(4 * NR_LANES))); // [ M*N ]
...
int verify_tensor(float *tensor1, float *tensor2, int64_t R,
                  int64_t C, int64_t D) {
...
void print_tensor(float *tensor, uint64_t num_rows,
...
printf("\nwindow is [%i x %i] \n", F, F);
	  ...
	  printf("----------------------------------------------------------------\n");
		...
		float output[(width / F)* (height / F) * channels]; // keep empty space at max size (just for the test)
		float golden_output[(width / F)* (height / F) * channels];
	  ...
	  printf("Computing result...\n");
	  	start_timer();
		fmax_pool32(output, input, height, width, channels, F);
		stop_timer();
...
printf("\nwindow is [%i x %i] \n", F, F);
	  ...
	  printf("Computing result...\n");
	  	start_timer();
		favg_pool32(output, input, height, width, channels, F);
		stop_timer();

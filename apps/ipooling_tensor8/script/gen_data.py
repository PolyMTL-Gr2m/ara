...
tensor = np.around(rand_tensor(L, M, N, -128)).astype(np.int8)
np.random.shuffle(tensor.flat)

# Create the empty o matrix
empty_o = np.zeros((L, M, N)).astype(np.int8)

M_o = int(M / 2)
N_o = int(N / 2)

result_max_2 = np.zeros((L, M_o, N_o)).astype(np.int8) #Num_filter x M x N
result_avg_2 = np.zeros((L, M_o, N_o)).astype(np.int8) #Num_filter x M x N
...
result_max_3 = np.zeros((L, M_o, N_o)).astype(np.int8) #Num_filter x M x N
result_avg_3 = np.zeros((L, M_o, N_o)).astype(np.int8) #Num_filter x M x N

...
void iconv2d_tensor8_wide(int32_t *o, int8_t *i, int8_t *f, int64_t R, int64_t C, int64_t W, int64_t F, int64_t K) {
  ...
  for(int64_t k = 0; k < K; k++) {
	  ...
	  {
	   case 1:
	  		iconv2d_tensor8_vec_8xC_1x1_wide(o_, i_, f_, R, C, W, F);
	  		break;
		case 3:
			iconv2d_tensor8_vec_8xC_3x3_wide(o_, i_, f_, R, C, W, F);
			break;
		case 5:
			iconv2d_tensor8_vec_4xC_5x5_wide(o_, i_, f_, R, C, W, F);
			break;
		case 7:
			iconv2d_tensor8_vec_2xC_7x7_wide(o_, i_, f_, R, C, W, F);
			break;
		}
...
void iconv2d_tensor8_vec_8xC_1x1_wide(int32_t *o, int8_t *i, int8_t *f, int64_t R, int64_t C, int64_t W, int64_t F) {
  ...
  int64_t const ldi = C;					// increment adress on input  loads 
  int64_t const next_kernel = 1;			// increment adress on filter loads
  ...
  for (int c = 0 ; c <= C - TILE_SIZE  ; c+= TILE_SIZE){ // IF CONVOLUTION NEED TO BE TILED (C > TILE_SIZE)
	  ...
	  for (int r = 0; r < R ; r += 8){
			  ...
			  asm volatile("lb %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t) : "r"(next_kernel));  
			  
			  asm volatile("vle8.v v17, (%0); add %0, %0, %1" : "+&r"(i__) : "r"(ldi));
			  ...
			  asm volatile("vle8.v v24, (%0); add %0, %0, %1" : "+&r"(i__) : "r"(next_plane_8));
			  
			  // Sign-extend SEW/2 source to SEW destination
	 
			  asm volatile("vsext.vf2 v16, v17"); 
			  asm volatile("vsext.vf2 v17, v18"); 
			  asm volatile("vsext.vf2 v18, v19"); 
			  asm volatile("vsext.vf2 v19, v20"); 
  ...
  if(size != 0) {
  	  ...
  	  for (int r = 0; r < R ; r += 8){
			  ...
			  int8_t * i__ = i_ + r * C; // increment the input 
			  f_ = f;
...
void iconv2d_tensor8_vec_8xC_3x3_wide(int32_t *o, int8_t *i, int8_t *f, int64_t R, int64_t C, int64_t W, int64_t F) {
  ...
  int64_t const ldi = (C + F - 1);
  int64_t const ldf = F;
  int64_t next_column = (F * (F - 1) - 1);
  int64_t const next_kernel = 1;
  ...
  for (int c = 0 ; c <= (C - TILE_SIZE_OUT) ; c += TILE_SIZE_OUT){ // IF CONVOLUTION NEED TO BE TILED (C > TILE_SIZE)
	  ...
	  asm volatile("vle8.v v28, (%0); add %0, %0, %1" : "+&r"(i__) : "r"(next_plane_8));
	  ...
	  asm volatile("lb %1, (%0); sub %0, %0, %2" : "+&r"(f_), "=&r"(t2) : "r"(next_column));
	  ...
	  asm volatile("lb %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t2) : "r"(next_kernel)); 
...
void iconv2d_tensor8_vec_4xC_5x5_wide(int32_t *o, int8_t *i, int8_t *f, int64_t R, int64_t C, int64_t W, int64_t F) {
  ...
  asm volatile("lb %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t0) : "r"(ldf));  
  asm volatile("lb %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t1) : "r"(ldf));
  asm volatile("lb %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t2) : "r"(ldf));  
  asm volatile("lb %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t3) : "r"(ldf));  
  asm volatile("lb %1, (%0); sub %0, %0, %2" : "+&r"(f_), "=&r"(t4) : "r"(next_column));
  ...
  asm volatile("lb %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t2) : "r"(ldf));  
  asm volatile("lb %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t3) : "r"(ldf));  
  asm volatile("lb %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t4) : "r"(next_kernel));
  ...
  // this part does all the block between the first 4 output rows and last 4 output rows
	  ...
	  asm volatile("vle8.v v22, (%0); add %0, %0, %1" : "+&r"(i__) : "r"(ldi));
	  asm volatile("vle8.v v23, (%0); add %0, %0, %1" : "+&r"(i__) : "r"(ldi));
	  asm volatile("vle8.v v24, (%0); add %0, %0, %1" : "+&r"(i__) : "r"(next_plane_4));	   
	  ...
	  for (int ch = 1; ch < W ; ch++){ 
		  ...
		  asm volatile("lb %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t0) : "r"(ldf)); //next column
	...
	// LAST 4 output rows (no need to precalculate the value of the next block)
		...
		o__ = o_ + (r_loads - F + 1) * C;
  		...
  		int64_t next_column_jump_1st_row = 3 * F - 1;
  		int64_t next_kernel_jump_1st_row = F + 1;
	  ...
	  asm volatile("lb %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t2) : "r"(ldf));
	  asm volatile("lb %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t3) : "r"(ldf));  
	  asm volatile("lb %1, (%0); sub %0, %0, %2" : "+&r"(f_), "=&r"(t4) : "r"(next_column_jump_1st_row)); 
	  ...
	  asm volatile("lb %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t2) : "r"(ldf));
	  asm volatile("lb %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t3) : "r"(ldf)); 
	  asm volatile("lb %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t4) : "r"(next_kernel_jump_1st_row));
...
void iconv2d_tensor8_vec_2xC_7x7_wide(int32_t *o, int8_t *i, int8_t *f, int64_t R, int64_t C, int64_t W, int64_t F){
  ...
  for (int ch = 0; ch < W ; ch ++){
	  ...
	  asm volatile("lb %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t4) : "r"(ldf));
	  ...
	  asm volatile("lb %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t5) : "r"(ldf)); 
	  ...
	  asm volatile("lb %1, (%0); sub %0, %0, %2" : "+&r"(f_), "=&r"(t6) : "r"(next_column));
	  ...
	  asm volatile("lb %1, (%0); add %0, %0, %2" : "+&r"(f_), "=&r"(t6) : "r"(next_kernel));

...
#define C_in 		64		// Number of input channels 
...
int64_t i[I_MAX * I_MAX * C_in];

int64_t f[F_MAX * F_MAX * C_in * C_out];

//////////////////////////////////////
// initialisation functions         //
//////////////////////////////////////

void iconv2d_tensor64_naive(int64_t *o, int64_t *i, int64_t *f, int64_t R, int64_t C, int64_t W, int64_t F, int64_t K) {

//treat pointers as 3D arrays
int64_t (*i_)[R+F-1][C+F-1] = (int64_t (*)[R+F-1][C+F-1])i;
int64_t (*f_)[W][F][F] = (int64_t (*)[W][F][F])f;
int64_t (*o_)[R][C] = (int64_t (*)[R][C])o;
...
void init_tensor(int64_t *tensor, int64_t R, int64_t C, int64_t D, int64_t precision) {

int64_t const mask = (1 << precision) - 1;
int64_t const val_init = R*C*D;
int64_t val = 0;
...
for(int64_t precA = 1; precA <= PRECA_MAX; precA++){
	for(int64_t precW = 1; precW <= PRECA_MAX; precW++){
		...
		for(int64_t F = 3 ; F <= F_MAX ; F += 2){
			...
			int64_t filter[filters * F * F * channels];
			...
			int64_t golden_o[(I_MAX - F + 1) * (I_MAX - F + 1) * C_out]; 
			...
			for(int size = I_START ; size <= I_MAX ; size *= 2){
				...
				printf("Result (64b) is an output of [1 x %d x %d x %d] \n", C_out, size - F + 1, size - F + 1);
				printf("----------------------------------------------------------------\n");
				printf("\n");
				...
				int64_t output[width * height * filters];
				...
				start_timer();	
				ibsconv2d_tensor64_3x3(output, input, filter, height, width, channels, F, filters, precA, precW);
				stop_timer();
				printf("                                                            done\n");                                
				...
				float performance = 4.0 * F * F * C_out * (size - F + 1) * (size - F + 1) * precA * precW / runtime;

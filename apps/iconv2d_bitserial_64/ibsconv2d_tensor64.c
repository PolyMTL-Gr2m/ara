...
void print_tensor_(uint64_t *tensor, uint64_t num_rows, uint64_t num_columns, uint64_t num_depth) {
  ...
  for (uint64_t k = 0; k < num_depth; ++k) {
      ...
      printf("%lX ", tensor[(i+k*num_rows) * num_columns  + j ]);
    	}
    	printf("\n");
  	 }
  	 printf("\n");
...
void print_tensor_32(uint32_t *tensor, uint64_t num_rows, uint64_t num_columns, uint64_t num_depth) {
  printf("0x%8X\n", (uint64_t)tensor);
  for (uint64_t k = 0; k < num_depth; ++k) {
  	for (uint64_t i = 0; i < num_rows; ++i) {
    	for (uint64_t j = 0; j < num_columns; ++j) {
...
void print_tensor_16(uint16_t *tensor, uint64_t num_rows, uint64_t num_columns, uint64_t num_depth) {
  printf("0x%8X\n", (uint64_t)tensor);
  for (uint64_t k = 0; k < num_depth; ++k) {
  	for (uint64_t i = 0; i < num_rows; ++i) {
    	for (uint64_t j = 0; j < num_columns; ++j) {
...
for (int height = H_per_it ; height <= H_in ; height += H_per_it){
	...
	asm volatile("vsetvli zero, %0, e64, m4, ta, ma" ::"r"(W_in_tiled << 1));
...
for (int height = H_per_it ; height <= H_in ; height += H_per_it){
	...
	for(int64_t h_i = 0 ; h_i < 2 ; h_i ++){
		...
		asm volatile("vse64.v v24, (%0)" : "+&r"(o_));

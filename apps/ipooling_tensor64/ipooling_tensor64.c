...
void imax_pool_vec_1xC_2x2(int64_t *o, int64_t *i, int64_t R, int64_t C, int64_t W, int64_t F, int64_t stride) {
	...
	asm volatile("vsetvli zero, %0, e64, m4, ta, ma" ::"r"(TILE_SIZE >> 1)); // we only load one of two values 
	
	for (int c = 0 ; c < C ; c += TILE_SIZE) // IF CONVOLUTION NEED TO BE TILED (C > TILE_SIZE)
	{
		if(c > C - TILE_SIZE)
			asm volatile("vsetvli zero, %0, e64, m4, ta, ma" ::"r"((C % TILE_SIZE) >> 1)); // we only load one of two values 

		int64_t * i_ = i + c; 			// input pointer realtive to the tile (constant throughout the tile)
		int64_t * o_ = o + (c >> 1);			// output pointer relative to the tile	
		
		//channel and height loop are fused to avoid branches
		for (int r = 0 ; r < R * W ; r += block_size_2x2){
			
		  	// Load F row of size C
		  	asm volatile("vlse64.v v16, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += 1;
		  	asm volatile("vlse64.v v20, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += C - 1;
		  	// next line
		  	asm volatile("vlse64.v v24, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += 1;
		  	asm volatile("vlse64.v v28, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += C - 1;
...
void imax_pool_vec_1xC_3x3(int64_t *o, int64_t *i, int64_t R, int64_t C, int64_t W, int64_t F, int64_t stride) {
	...
	// vsetvli C (in order to load everything)
	asm volatile("vsetvli zero, %0, e64, m2, ta, ma" ::"r"(TILE_SIZE_3x3 / 3)); // we only load one of two values 
	
	for (int c = 0 ; c < C ; c += TILE_SIZE_3x3) // IF CONVOLUTION NEED TO BE TILED (C > TILE_SIZE)
	{
		if(c > C - TILE_SIZE_3x3)
			asm volatile("vsetvli zero, %0, e64, m2, ta, ma" ::"r"((C % TILE_SIZE_3x3) / 3)); // we only load one of two values 

		int64_t * i_ = i + c; 			// input pointer realtive to the tile (constant throughout the tile)
		int64_t * o_ = o + c / 3;			// output pointer relative to the tile	
		
		//channel and height loop are fused to avoid branches
		for (int r = 0 ; r < R * W ; r += block_size_3x3){
			
		  	// Load F row of size C
		  	asm volatile("vlse64.v v10, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += 1;
		  	asm volatile("vlse64.v v12, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += 1;
		  	asm volatile("vlse64.v v14, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += C - 2;
		  	
		  	// next line
		  	asm volatile("vlse64.v v16, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += 1;
		  	asm volatile("vlse64.v v18, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += 1;
		  	asm volatile("vlse64.v v20, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += C - 2;
		  	
		  	// next line
		  	asm volatile("vlse64.v v22, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += 1;
		  	asm volatile("vlse64.v v24, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += 1;
		  	asm volatile("vlse64.v v26, (%0), %1" : "+&r"(i_) : "r"(ld));
		  	i_ += C - 2;
...
void iavg_pool_vec_1xC_2x2(int64_t *o, int64_t *i, int64_t R, int64_t C, int64_t W, int64_t F, int64_t stride) {
	...
	{
		if(c > C - TILE_SIZE)
			asm volatile("vsetvli zero, %0, e64, m4, ta, ma" ::"r"((C % TILE_SIZE) >> 1));
		...
		for (int r = 0 ; r < R * W ; r += block_size_2x2){
		 	...
		 	// max function between the F rows
		 	asm volatile("vadd.vv v0,  v16,  v20");
		 	asm volatile("vadd.vv v8,  v24,  v28");
		 	
		 	asm volatile("vadd.vv v8, v0,  v8");
		 	
		 	asm volatile("vdiv.vx v0, v8, %0" : "+&r"(avg_div));
...
void iavg_pool_vec_1xC_3x3(int64_t *o, int64_t *i, int64_t R, int64_t C, int64_t W, int64_t F, int64_t stride) {
	...
	{
		if(c > C - TILE_SIZE_3x3)
			asm volatile("vsetvli zero, %0, e64, m2, ta, ma" ::"r"((C % TILE_SIZE_3x3) / 3));
		...
		for (int r = 0 ; r < R * W ; r += block_size_3x3){
		  	...
		  	// first two lines
		 	asm volatile("vadd.vv v0,  v12,  v14");
		 	asm volatile("vadd.vv v4,  v18,  v20");
		 	asm volatile("vadd.vv v8,  v24,  v26");
 			
 			// add all the 9 registers
 			
 			asm volatile("vadd.vv v0, v0, v16");
 			asm volatile("vadd.vv v4, v4, v22");
 			asm volatile("vadd.vv v8, v8, v28");
		 	...
		 	asm volatile("vdiv.vx v0, v0, %0" : "+&r"(avg_div));

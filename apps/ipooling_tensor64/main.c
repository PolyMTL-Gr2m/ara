...
extern int64_t golden_o_max_2 [] __attribute__((aligned(4 * NR_LANES))); // [ M*N ]
extern int64_t golden_o_avg_2 [] __attribute__((aligned(4 * NR_LANES))); // [ M*N ]

extern int64_t golden_o_max_3 [] __attribute__((aligned(4 * NR_LANES))); // [ M*N ]
extern int64_t golden_o_avg_3 [] __attribute__((aligned(4 * NR_LANES))); // [ M*N ]
...
void print_tensor(int64_t *tensor, uint64_t num_rows,
...
int main() {
  ...
  printf("\n");
  printf("=================\n");
  printf("= MAXPOOL int64 =\n");
  printf("=================\n");
  printf("\n");
  printf("Format is NCHW\n");
  printf("\n");
  
int64_t * golden_o_max;
...
printf("\nwindow is [%i x %i] \n", F, F);
	  ...
	  printf("----------------------------------------------------------------\n");
		...
		int64_t output[(width / F)* (height / F) * channels]; // keep empty space at max size (just for the test)
		int64_t golden_output[(width / F)* (height / F) * channels];
	  ...
	  printf("Computing result...\n");
	  	start_timer();
		imax_pool64(output, input, height, width, channels, F);
		stop_timer();
  ...
  printf("\n");
  printf("=================\n");
  printf("= AVGPOOL int64 =\n");
  printf("=================\n");
  printf("\n");
  printf("Format is NCHW\n");
  printf("\n");
  
int64_t * golden_o_avg;
...
printf("\nwindow is [%i x %i] \n", F, F);
	  ...
	  printf("Computing result...\n");
	  	start_timer();
		iavg_pool64(output, input, height, width, channels, F);
		stop_timer();

...
tensor = np.around(rand_tensor(L, M, N, -128)).astype(np.int64) >> 4
...
result_max_2 = np.zeros((L, M_o, N_o)).astype(np.int64) #Num_filter x M x N
result_avg_2 = np.zeros((L, M_o, N_o)).astype(np.int64) #Num_filter x M x N
...
result_max_3 = np.zeros((L, M_o, N_o)).astype(np.int64) #Num_filter x M x N
result_avg_3 = np.zeros((L, M_o, N_o)).astype(np.int64) #Num_filter x M x N
...
if memlayout == "NHWC":
	result_NWHC = np.zeros((M, N, num_filter)).astype(np.int64) 
	tensor_NWHC = np.zeros((1, M_pad, N_pad, L)).astype(np.int64) 
	gen_filter_NWHC = np.zeros((num_filter,filter_size, filter_size, L_pad)).astype(np.int64) 

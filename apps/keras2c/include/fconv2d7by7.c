...
void fconv2d7by7(double *o, double *i, double *f, int64_t R, int64_t C,
  ...
  f41 = f[41];
  f48 = f[48];
  printf("finished setting up f array \n");
  ...
  printf("Start calculating the next pointers to the elements to be slided in \n");
  // Start calculating the next pointers to the elements to be slided in
  i_slide_ptr_0 = i + C + 0 * (C + F - 1);
  i_slide_ptr_1 = i + C + 1 * (C + F - 1);
  i_slide_ptr_2 = i + C + 2 * (C + F - 1);

  printf("Main kernel, last iteration with filter coefficients reuse \n");
  ...
  // Row 4 -> 6 //
  ////////////////
  printf("Main kernel, unrolled by 2 \n");
  // Main kernel, unrolled by 2
  for (int k = 0; k < F / 2; ++k) {
  ...
  printf("Main kernel, last iteration with filter coeficients \n");
  // Main kernel, last iteration with filter coefficients reuse
  ...
  asm volatile("vle64.v v0, (%0); add %0, %0, %1" : "+&r"(i) : "r"(ldi_pad));
  printf("starting main algorithm loop\n" );
  ...
  // Row (I-1)-3 -> (I-1) //
  //////////////////////////
  printf(" main kernel divided by 2\n");
  // Main kernel, unrolled by 2
  for (int k = 0; k < F / 2; ++k) {
    asm volatile("vfslide1down.vf v0, v2, %0" ::"f"(*i_slide_ptr_0++));

...
int main() {
  ...
  for(int size = 8 ; size <= I_MAX ; size = size * 2){
	  ...
	  printf("----------------------------------------------------------------\n");
		...
		int32_t output[width * height * channels]; // keep empty space at max size (just for the test)
		int32_t golden_output[width * height  * channels];
	  ...
	  printf("Computing result...\n");
	  	start_timer();
		iReLu_tensor32(output, input, height, width, channels);
		stop_timer();

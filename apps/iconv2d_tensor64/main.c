...
extern int64_t f[] __attribute__((aligned(4 * NR_LANES))); // [K x F x F x L]
...
extern int64_t golden_o_1[] __attribute__((aligned(4 * NR_LANES)));
extern int64_t golden_o_3[] __attribute__((aligned(4 * NR_LANES)));
extern int64_t golden_o_5[] __attribute__((aligned(4 * NR_LANES)));
extern int64_t golden_o_7[] __attribute__((aligned(4 * NR_LANES)));
...
void print_tensor(int64_t const *tensor, uint64_t num_rows,
...
for(int64_t F = 1 ; F <= F_MAX ; F += 2){
	...
	start_timer();
	#endif			
	iconv2d_tensor64(output, input, filter, height, width, channels, F, filters);
	#ifndef SPIKE
	stop_timer();
	#endif

...
int main() {
  ...
  for(int size = 8 ; size <= I_MAX ; size = size * 2){
	  ...
	  printf("----------------------------------------------------------------\n");
		...
		float output[width * height * channels]; // keep empty space at max size (just for the test)
		float golden_output[width * height  * channels];
	  ...
	  printf("Computing result...\n");
	  	start_timer();
		fReLu_tensor32(output, input, height, width, channels);
		stop_timer();

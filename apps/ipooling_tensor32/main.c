...
extern int32_t i[] __attribute__((aligned(4 * NR_LANES))); // [ (M+floor(F/2)) * (N+floor(F/2)) ]
extern int32_t o[] __attribute__((aligned(4 * NR_LANES)));        // [ M*N ]

extern int32_t golden_o_max_2 [] __attribute__((aligned(4 * NR_LANES))); // [ M*N ]
extern int32_t golden_o_avg_2 [] __attribute__((aligned(4 * NR_LANES))); // [ M*N ]

extern int32_t golden_o_max_3 [] __attribute__((aligned(4 * NR_LANES))); // [ M*N ]
extern int32_t golden_o_avg_3 [] __attribute__((aligned(4 * NR_LANES))); // [ M*N ]
...
int verify_tensor(int32_t *tensor1, int32_t *tensor2, int64_t R,
                  int64_t C, int64_t D) {
...
void print_tensor(int32_t *tensor, uint64_t num_rows,
...
int main() {
  ...
  printf("\n");
  printf("=================\n");
  printf("= MAXPOOL int32 =\n");
  printf("=================\n");
  printf("\n");
  printf("Format is NCHW\n");
  printf("\n");
  
int32_t * golden_o_max;
...
printf("\nwindow is [%i x %i] \n", F, F);
	  ...
	  printf("----------------------------------------------------------------\n");
		...
		int32_t output[(width / F)* (height / F) * channels]; // keep empty space at max size (just for the test)
		int32_t golden_output[(width / F)* (height / F) * channels];
	  ...
	  printf("Computing result...\n");
	  	start_timer();
		imax_pool32(output, input, height, width, channels, F);
		stop_timer();
  ...
  printf("\n");
  printf("=================\n");
  printf("= AVGPOOL int32 =\n");
  printf("=================\n");
  printf("\n");
  printf("Format is NCHW\n");
  printf("\n");
  
int32_t * golden_o_avg;
...
printf("\nwindow is [%i x %i] \n", F, F);
	  ...
	  printf("Computing result...\n");
	  	start_timer();
		iavg_pool32(output, input, height, width, channels, F);
		stop_timer();

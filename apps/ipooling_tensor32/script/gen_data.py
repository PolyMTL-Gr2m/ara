...
tensor = np.around(rand_tensor(L, M, N, -128)).astype(np.int32)
np.random.shuffle(tensor.flat)

# Create the empty o matrix
empty_o = np.zeros((L, M, N)).astype(np.int32)

M_o = int(M / 2)
N_o = int(N / 2)

result_max_2 = np.zeros((L, M_o, N_o)).astype(np.int32) #Num_filter x M x N
result_avg_2 = np.zeros((L, M_o, N_o)).astype(np.int32) #Num_filter x M x N
...
result_max_3 = np.zeros((L, M_o, N_o)).astype(np.int32) #Num_filter x M x N
result_avg_3 = np.zeros((L, M_o, N_o)).astype(np.int32) #Num_filter x M x N
...
if memlayout == "NHWC":
	result_NWHC = np.zeros((M, N, num_filter)).astype(np.int32) 
	tensor_NWHC = np.zeros((1, M_pad, N_pad, L)).astype(np.int32) 
	gen_filter_NWHC = np.zeros((num_filter,filter_size, filter_size, L_pad)).astype(np.int32) 

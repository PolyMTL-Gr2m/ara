...
#define PRECISION_DOUBLE 0.00001
...
int verify_d64tensor(double *tensor1, double *tensor2, int64_t R,
                  int64_t C, int64_t D) {
...
void print_d64tensor(double const *tensor, uint64_t num_rows,
...
int main() {
  ...
  start_timer();
  //if (F == 3)
    fconv2d_tensor64_3x3(o, i, f, M, N, L, F, K);
  /*else if (F == 5)
  ...
  */stop_timer();
  ...
  int error = verify_d64tensor(o, golden_o, M, N, K);
  if (error != 0) {
    printf("Fail.\n");
  } else {
    printf("Passed.\n");
